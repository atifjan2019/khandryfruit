...
export default async function PackagingPage() {
  ...
  const options = await db.giftPackagingOption.findMany({
    ...
    <div className="admin-page-v2">
      <div className="admin-page-heading">
        <div>
          ...
          <h1>Packaging</h1>
          <p>Bilingual packaging choices and their customer-visible charges.</p>
        </div>
      </div>
      {options.map((option) => (
        <AdminForm
          key={option.id}
          action={upsertPackagingAction}
          submitLabel="Save packaging"
        >
          <input type="hidden" name="packagingId" value={option.id} />
          <AdminSection
            title={`${option.nameEn} · ${formatMoney(option.priceCents, "en")}`}
          >
            <div className="admin-field-grid">
              <Field
                label="German name"
                name="nameDe"
                defaultValue={option.nameDe}
                required
              />
              <Field
                label="English name"
              ...
              <TextField
                ...
                name="descriptionDe"
                defaultValue={option.descriptionDe}
              />
              <TextField
              ...
              <Field
                ...
                type="number"
                min={0}
                defaultValue={option.priceCents}
                required
              />
              <Field
                label="Sort order"
      ...
      <AdminForm action={upsertPackagingAction} submitLabel="Create packaging">
        <AdminSection title="New packaging option">
          <div className="admin-field-grid">
