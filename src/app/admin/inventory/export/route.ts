...
export async function GET() {
  ...
  const header =
    "product,sku,weight_grams,on_hand,reserved,available,low_stock_threshold";
  const body = rows.map((item) =>
    [
      item.variant.product.translations[0]?.name ??
        item.variant.product.internalName,
      item.variant.sku,
  ...
  return new Response([header, ...body].join("\r\n"), {
    headers: {
      ...
      "content-disposition": "attachment; filename=khan-inventory.csv",
    },
  });
}
