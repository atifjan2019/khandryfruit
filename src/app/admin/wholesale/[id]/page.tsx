      ...
      <div className="admin-two-column">
        <section className="admin-card">
          ...
          <dl className="admin-summary">
            ...
            <dd>{application.monthlyOrderVolume}</dd>
            <dt>Products</dt>
            <dd>{application.productsOfInterest.join(", ") || "—"}</dd>
            <dt>Delivery countries</dt>
            <dd>{application.deliveryCountries.join(", ") || "—"}</dd>
            <dt>VAT ID</dt>
            <dd>{application.vatId ?? "—"}</dd>
            <dt>Message</dt>
            <dd>{application.message ?? "—"}</dd>
          </dl>
        </section>
        <AdminForm action={decideWholesaleAction} submitLabel="Save decision">
          <input type="hidden" name="applicationId" value={application.id} />
          <AdminSection
            title="Decision"
            description="The previous and new status are audited with the acting administrator."
          >
            <SelectField
              label="Status"
              name="status"
            ...
            <Field
              ...
              min={0}
              defaultValue={
                application.user?.wholesaleAccount?.minimumOrderCents
              }
            />
            <Checkbox
              ...
              defaultChecked={
                application.user?.wholesaleAccount?.invoicePaymentEligible
              }
            />
            <TextField
