...
export const auth = betterAuth({
  ...
  trustedOrigins: [env.NEXT_PUBLIC_SITE_URL, env.BETTER_AUTH_URL, vercelOrigin].filter(
  ...
  emailAndPassword: {
    ...
    sendResetPassword: async ({ user, url }) => {
      ...
      await getEmailProvider().send({
        to: user.email,
        locale: german ? "de" : "en",
        subject: german ? "Passwort zurücksetzen – Khan Dry Fruit" : "Reset your password – Khan Dry Fruit",
        text: german ? `Setzen Sie Ihr Passwort über diesen Link zurück: ${url}` : `Reset your password using this link: ${url}`,
  ...
  emailVerification: {
    ...
    sendVerificationEmail: async ({ user, url }) => {
      ...
      await getEmailProvider().send({
        to: user.email,
        locale: german ? "de" : "en",
        subject: german ? "E-Mail bestätigen – Khan Dry Fruit" : "Verify your email – Khan Dry Fruit",
        text: german ? `Bestätigen Sie Ihre E-Mail-Adresse: ${url}` : `Verify your email address: ${url}`,
