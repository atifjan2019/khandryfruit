...
describe("rate limiter", () => {
  ...
  it("allows requests up to the limit and then blocks", async () => {
    ...
    const now = 1_000_000;
    expect((await checkRateLimit("contact:1.2.3.4", options, now)).allowed).toBe(true);
    expect((await checkRateLimit("contact:1.2.3.4", options, now + 1)).allowed).toBe(true);
    expect((await checkRateLimit("contact:1.2.3.4", options, now + 2)).allowed).toBe(true);
  ...
  it("frees capacity once the window has passed", async () => {
    ...
    expect((await checkRateLimit("key", options, now + 500)).allowed).toBe(false);
    expect((await checkRateLimit("key", options, now + 1_500)).allowed).toBe(true);
  });
});
